...
function btnCheckout() {
    ...
    jQuery(document.body).on('updated_cart_totals', function (event) {
        ...
        var flycartBtn = document.getElementsByClassName(
            ...
            if (stickyBtn != null) {
                stickyBtn.onclick = function () {
                    ...
                    const customFieldForm =
                        document.getElementsByClassName('wcpa_form_outer');

                    if (customFieldForm && customFieldForm.length > 0) {
                        var customProductFieldForm = customFieldForm[0];

                        var fieldValues =
                            customProductFieldForm.getElementsByTagName('input');
            ...
            return new Promise(function (resolve, reject) {
                ...
                xhr.onload = function () {
                    if (this.status === 200) {
                        ...
                        reject({
                            status: this.status,
                            response: rzp1cc.parseIfJson(this.response),
                        });
                    }
                };
        ...
        sendCartData: function (body, e) {
            ...
            if (body.pdpCheckout) {
                rzp1cc
                    ...
                    .then((cartResponse) => {
                        return Promise.all([
                            initRazorpayWithOptions(cartResponse),
                ...
                Promise.all([
                    rzp1cc.makeRequest(rzp1cc.cartApi, body).then((cartResponse) => {
                        return initRazorpayWithOptions(cartResponse);
                    }),
                    rzp1cc.makeRequest(rzp1cc.orderApi, body),
                ])
                    .then(updateLazyOrder)
    ...
    function handleCheckoutError(e) {
        rzp1cc?.razorpayInstance?.close();

        // Response sent to the User when cart is empty or order creation fails
        if (e.status == 400) {
            if (e.response.code == 'BAD_REQUEST_EMPTY_CART') {
                document.getElementById('error-message').innerHTML =
                    "<p style='margin-top: revert;text-color: #e2401c !important;color: #e80707;'>Order could not be placed as your cart is empty.</p>";
            } else if (e.response.code == 'ORDER_CREATION_FAILED') {
                document.getElementById('error-message').innerHTML =
                    "<p style='margin-top: revert;text-color: #e2401c !important;color: #e80707;'>Razorpay Error: Order could not be placed, please try again after sometime.</p>";
            } else if (
                e.response.code == 'MIN_CART_AMOUNT_CHECK_FAILED' ||
                e.response.code == 'WOOCOMMERCE_ORDER_CREATION_FAILED'
            ) {
                document.getElementById('error-message').innerHTML =
                    "<p style='margin-top: revert;text-color: #e2401c !important;color: #e80707;'>" +
                    e.response.message +
                    '</p>'; // nosemgrep: insecure-innerhtml
            } else {
                document.getElementById('error-message').innerHTML =
                    "<p style='margin-top: revert;text-color: #e2401c !important;color: #e80707;'>Something went wrong, please try again after sometime.</p>";
            }
        } else {
            document.getElementById('error-message').innerHTML =
                "<p style='margin-top: revert;text-color: #e2401c !important;color: #e80707;'>Something went wrong, please try again after sometime.</p>";
    ...
    function updateLazyOrder([razorpayCheckout, orderResponse]) {
        ...
        razorpayCheckout.set('callback_url', orderResponse.callback_url);
        razorpayCheckout.set('notes', orderResponse.notes);
        razorpayCheckout.set('description', orderResponse.description);
    }
    ...
    if (btnPdp !== null) {
        btnPdp.addEventListener('click', openPdpRzpCheckout);
    }
    ...
    function initRazorpayWithOptions(cartResponse) {
        rzp1cc.showSpinner(false);
        ...
        try {
            ...
            rzp1cc.razorpayInstance = razorpayCheckout;
